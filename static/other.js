...
// Escuchar atributos seleccionados del árbol jerárquico radial
...
function updateChart() {
    ...
    const selectedAttributes = Array.from(document.querySelectorAll('.options-chek input[type="checkbox"]:checked'))
                                   .map(cb => cb.value);
    const selectedDendrogramAttributes = Array.from(document.querySelectorAll('.options-chek-correlation input[type="checkbox"]:checked'))
                                              .map(cb => cb.value);
    const startDate = document.getElementById('fecha-inicio').value;
    const endDate = document.getElementById('fecha-fin').value;

    if (selectedCities.length === 0 || selectedAttributes.length === 0 || selectedDendrogramAttributes.length === 0) return;
    ...
    selectedCities.forEach(selectedCity => {
        d3.csv(`data/${selectedCity}`).then(data => {
            ...
            drawRadialDendrogram(parsedData, selectedDendrogramAttributes, startDate, endDate);
...
// Funciones para cálculos de correlación y distancia
    ...
    for (let i = 0; i < numAttributes; i++) {
        for (let j = 0; j < numAttributes; j++) {
            correlationMatrix[i][j] = calculateCorrelation(
                data.map(row => row[selectedAttributes[i]]),
                data.map(row => row[selectedAttributes[j]])
            );
        }
    }
...
function createDendrogramHierarchy(distanceMatrix, attributes) {
    return {
        name: "root",
        children: attributes.map((attr, i) => ({ name: attr }))
    };
}

function drawRadialDendrogram(data, selectedAttributes, startDate, endDate) {
    d3.select("#chart-view-dendrogram").html("");
    const attributeData = data.map(d => selectedAttributes.map(attr => d[attr]));

    const correlationMatrix = calculateCorrelationMatrix(data, selectedAttributes);
    const distanceMatrix = calculateDistanceMatrix(correlationMatrix);
    // Imprimir la matriz de correlación en la consola
    console.log("Matriz de Correlación de los Contaminantes Seleccionados:");
    ...
    const root = d3.hierarchy(createDendrogramHierarchy(distanceMatrix, selectedAttributes));

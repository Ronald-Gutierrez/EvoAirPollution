...
async function drawThemeRiver(cityFile, dates) {
    ...
    // Agregar etiquetas en inicio, medio y final del flujo, con límite de tamaño
    ...
    series.forEach(layer => {
        ...
        // Cálculo proporcional de índices basado en el rango de fechas
        const proportionalPositions = [
            { index: 0, anchor: "start" }, // Inicio
            { index: Math.floor(totalLength / 2), anchor: "middle" }, // Medio
        ...
        proportionalPositions.forEach(({ index, anchor }) => {
            ...
            if (Math.abs(height) > minHeightThreshold) {
                svg.append("text")
                    .attr("x", x(point.data.date)) // Posición X correctamente calculada
                    .attr("y", y((point[0] + point[1]) / 2)) // Posición Y en el centro de la capa
                    .text(layer.key)
                    .attr("fill", "black")
                    .attr("font-size", "12px")
                    .attr("font-weight", "bold")
